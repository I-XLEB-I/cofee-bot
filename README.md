...
Для точек, требующих проверки, бот показывает длительность паузы и последний
напиток. Кофейный состав имеет три состояния (`с кофе`, `без кофе`,
`не определено`) и определяется только по точному владельцем подтверждённому
справочнику рецептов — без догадок по названию или ИИ. Количество, время и
напиток берутся из Telemetron, состояние связи — из Vendista. Архивная точка
`Бел2` остаётся в истории обслуживания, но не включается в оперативный блок.
Если онлайн-сервис временно недоступен, текущая информация об обслуживании
продолжает работать.
